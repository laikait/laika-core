...
class Template
{
    ...
    {
        $this->templateDirectory = $this->resolvePath($subdir, $this->templateBase());
    ...
    {
        $base = APP_PATH . DS . 'lf-storage' . DS . 'cache' . DS . 'template';
        $this->cacheDirectory = $this->resolvePath($subdir, $base);
    ...
    protected function resolvePath(?string $path, string $base): string
    {
        ...
        $path = rtrim(str_replace(['/', '\\'], DS, (string) $path), DS);
    ...
    protected function loadFunctions(): void
    {
        ...
        File::require($root, true);

        $own = $this->templateDirectory . DS . 'loader.php';
        if ($own !== $root && File::exists($own)) {
            File::require($own, true);
        }
    }
